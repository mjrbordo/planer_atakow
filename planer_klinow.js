...
function rysujPlaner(){
	...
	if(game_data.screen=="info_village"){
		if(!mobile){
			...
			// POPRAWKA: wioska ze skórką ma koordynaty o 1 wiersz niżej niż standardowa
			// szukamy wzorca X|Y w rows[2] lub rows[3]
			var celTmp = tabela.rows[2].cells[1].textContent.match(/\d+\|\d+/);
			if(!celTmp) celTmp = tabela.rows[3].cells[1].textContent.match(/\d+\|\d+/);
