...
function rysujPlaner(){
	...
	if(game_data.screen=="info_village"){
		if(!mobile){
			// POPRAWKA: szukamy komórki z tekstem "Współrzędne:" i bierzemy następną komórkę
			// działa niezależnie od skórki, bo etykieta jest zawsze obecna
			var komorki = document.getElementById("content_value").getElementsByTagName("td");
			for(var k = 0; k < komorki.length; k++){
				if(komorki[k].textContent.trim() === "Współrzędne:"){
					var celTmp = komorki[k+1].textContent.trim().match(/\d+\|\d+/);
					if(celTmp) cel = celTmp[0];
					break;
				}
			}
		}
