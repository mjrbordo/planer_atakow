...
function rysujPlaner(){
	...
	if(game_data.screen=="info_village"){
		if(!mobile){
			// POPRAWKA: zamiast szukać po stałej pozycji wiersza/komórki (podatne na skórki),
			// szukamy wzorca X|Y w całym HTML kontenera – działa niezależnie od skórki
			var kontener = document.getElementById("content_value");
			if(kontener){
				var match = kontener.innerHTML.match(/\b(\d{1,3})\|(\d{1,3})\b/);
				if(match) cel = match[0];
			}
		}
		else{
